 ...
 * 
 * Name : Jinal Parikh   ID:201301017 
 * Email: 201301017@daiict.ac.in
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
void eval(char *cmdline) 
{
    char *argv[MAXARGS];
    pid_t pid;
    sigset_t sSet;   //declare a signal set variable and initialize it to be empty
    int state=parseline(cmdline,argv);/* parseline returns 1 if BG and 0 if FG job*/
    if(strlen(cmdline)==1)      //if no argument passed, simply return
          return;
    if (!builtin_cmd(argv)) //if not a builtin command
    {
         sigemptyset(&sSet);
         sigaddset(&sSet,SIGCHLD);   //Add the SIGCHLD signal to the signal set
         sigprocmask(SIG_BLOCK, &sSet, NULL); //block SIGCHLD before forking a child
	...
	sigprocmask(SIG_UNBLOCK, &sSet, NULL); //unblock the SIGCHLD signal
        if (execvp(argv[0], argv) < 0) { //returns -1 on error
          printf("%s: Command not found.\n", argv[0]);
          exit(0);
        }
       }
       else{         /* parent waits for foreground job to terminate */
        if (!state) {     //if foreground job
...
/*
 * has_stopped -
 * returns 1 if there are any stopped jobs
 * returns 0 if there aren't any stopped jobs
 */
int has_stopped() {
	int i;
	for (i = 0; i < MAXJOBS; i++) {
		if (jobs[i].state == ST) { //check if any stopped job
		   return 1;
		}
	}
	return 0;
...
int builtin_cmd(char **argv) 
{
	...
	if(has_stopped()==1){ //if any stopped job, print the message and return 1
...
void sigint_handler(int sig) 
{
   ...
   if(pid>0)
   {
     kill(-pid,SIGINT); //sending SIGINT signal to the foreground process group
   }
   return;
}
...
void sigtstp_handler(int sig) 
{
   ...
   if(pid>0)
   {
     kill(-pid,SIGTSTP); //sending SIGSTP signal to the foreground process group
   }
   return;
}
